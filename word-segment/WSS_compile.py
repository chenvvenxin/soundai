...
def getResponse():
    ...
    f = open('temp.txt', mode='a+', encoding='GBK')
    f.write('query:' + ipt + '\tanswer:' + res +'\n')
    f.close
...
#显示图片部分
#load = Image.open('soundai.jpg')
#re = ImageTk.PhotoImage(load)
#img = tk.Label(image=re)
#img.image = re
#img.place(x=0,y=0)

a = tk.Button(top, text = 'run',width=13, height=1, command = getResponse).place(x=260,y=230)
